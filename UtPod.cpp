...
int UtPod::addSong(Song const &s){
    ...
    if (getRemainingMemory() > s.getSongSize()) {
      ...
      /*  // DEBUG //
        ...
        cout << "No memory space to add song." << endl;
        return NO_MEMORY;
    }
          ...
          // cout << "Traverse linked list to remove song" << endl;
           ...
           while (before->next != NULL) {
              ...
              /* cout << "searching: " << beforeTitle << ", ";
               ...
               if ((beforeArtist == s.getArtist()) && (beforeTitle == s.getTitle()) &&
                   (beforeSize == s.getSongSize())) {

                   if (before->next->next != NULL) {
                       //cout << "delete Not last node" << endl;
...
void UtPod::shuffle(){
    ...
    //swapCtr = (rand() % songSum) + 1;
    swapCtr = 5;
    int j = 0;

    while(j < 10) {
        //  for(int j=0;j<songSum;j++) {

       // currentTime =  (unsigned)time(0);

        while (songindex1 == songindex2) {
            songindex1 = (rand() % songSum) + 1;
            songindex2 = (rand() % songSum) + 1;

        }

        cout << "index1: " << songindex1 << ", index2: " << songindex2 << endl;

        for (int i = 0; i < songindex1; i++) {
            ptr1 = ptr1->next;
        }
